...
var phpFreeChat = function (pfc, $, window) {
  function Plugin(element, options) {
    ...
    options && options.serverUrl || (options.serverUrl = defaults.serverUrl),
    options && options.packageUrl || (options.packageUrl = options.serverUrl + '/../package.json'),
    options && options.serverCheckUrl || (options.serverCheckUrl = options.serverUrl + '/../check.php'),
...
phpFreeChat = function (pfc, $) {
  return pfc.login = function (credentials, callback) {
    ...
    $.ajax({
      ...
      err && err.error && 40301 != err.errorCode ? pfc.showAuthForm(err.error)  : pfc.showAuthForm(),
  ...
  pfc.logout = function (callback) {
    $.ajax({
      ...
      $(pfc.element).trigger('pfc-logout', [
        pfc,
        userdata
      ]),
      callback && callback(null, userdata)
  ...
  pfc.showAuthForm = function (msg) {
    pfc.modalbox.open('<form>  <div class="popup-login">    <input type="text" name="login" placeholder="Login"/><br/>    <input type="submit" name="submit" value="Sign in" />' + (msg ? '<p>' + msg + '</p>' : '') + '  </div>' + '</form>').submit(function () {
...
phpFreeChat = function (pfc, $) {
  ...
  pfc.commands.unban = {
    ...
    receive: function (msg) {
      var cid = msg.recipient.split('|') [1];
      msg.body = msg.body.name + ' was unbanned by ' + msg.body.opname + ' from #' + pfc.getNameFromCid(cid),
  ...
  pfc.commands.banlist = {
    ...
    receive: function (msg) {
      ...
      $.each(msg.body, function (key, value) {
        value.timestamp = new Date(1000 * value.timestamp),
        banlist_txt.push(key + ' (banned by ' + value.opname + ' for ' + (value.reason ? 'the reason "' + value.reason + '"' : 'no reason') + ' on ' + value.timestamp + ')')
      }),
      msg.body = banlist_txt.length > 0 ? 'Banished list on #' + pfc.getNameFromCid(cid) + '\n  - ' + banlist_txt.join('\n  - ')  : 'Empty banished list on  #' + pfc.getNameFromCid(cid),
      pfc.appendMessage(msg)
    }
  },
  pfc
...
phpFreeChat = function (pfc, $) {
  ...
  pfc.commands.join = {
    ...
    receive: function (msg) {
      ...
      msg.body.op && pfc.addUidToCidOp(msg.sender, cid),
    ...
    if ($.each(pfc.commands, function (c) {
      if (RegExp('^/' + c + '( |$)').test(raw)) {
        ...
        $.each(pfc.commands[c].regexp, function (i, regexp) {
          ...
          null === cmd_arg && cmd_arg_tmp && cmd_arg_tmp.length > 0 && (cmd_arg = {
      ...
      cid: pfc.cid,
      message: raw
    }), null === cmd_arg) throw [cmd,
    pfc.commands[cmd].usage];
    return cmd_arg.cid || (cmd_arg.cid = cmd_arg.channel ? pfc.getCidFromName(cmd_arg.channel)  : pfc.cid),
    [
      cmd,
      cmd_arg
    ]
...
phpFreeChat = function (pfc, $, window, undefined) {
  return pfc.readPendingMessages = function (loop) {
    pfc.readPendingMessages.nb_network_error === undefined && (pfc.readPendingMessages.nb_network_error = 0),
    $.ajax({
      type: 'GET',
      ...
      $.each(msgs, function (i, m) {
        pfc.commands[m.type] !== undefined ? pfc.commands[m.type].receive(m)  : pfc.showErrorsPopup(['Comando desconhecido ' + m.type])
      }),
      loop && setTimeout(function () {
        pfc.readPendingMessages(!0)
      }, pfc.options.refresh_delay)
    }).error(function () {
      pfc.readPendingMessages.nb_network_error++ > pfc.options.tolerated_network_errors ? pfc.showErrorsPopup(['Erro de rede. Por favor recarregue o chat para continue.'])  : loop && setTimeout(function () {
        pfc.readPendingMessages(!0)
      }, pfc.options.refresh_delay)
    })
  },
  ...
  pfc.appendUser = function (user) {
    ...
    user.name && html.find('div.name').text(user.name),
    0 === users_dom.find('li').length && html.addClass('first'),
    html.find('div.status').addClass(user.active ? 'st-active' : 'st-inactive'),
    user.op && html.find('div.status').addClass('st-op');
    var userids = [
    ];
    if ($(pfc.element).find('div.pfc-users li.user').each(function (i, dom_user) {
      userids.push(parseInt($(dom_user).attr('id').split('_') [1], 10))
    }), 0 === user.id) do user.id = Math.round(10000 * Math.random());
    while ( - 1 !== $.inArray(user.id, userids));
  ...
  pfc.appendMessage = function (msg) {
    ...
    msg.date = new Date(1000 * msg.timestamp).toLocaleTimeString(),
    'join' == msg.type ? msg.body = msg.name + ' entrou no canal' : 'leave' == msg.type && (msg.body = msg.name + ' saiu do canal' + (msg.body ? ' (' + msg.body + ')' : ''));
    var groupmsg_dom = $(pfc.element).find('.pfc-messages .messages-group:last'),
    messages_dom = $(pfc.element).find('.pfc-messages'),
    html = null;
    groupmsg_dom.attr('data-from') != msg.from && (html = $('<div class="messages-group" data-stamp="" data-from="">       <div class="date"></div>       <div class="name"></div>     </div>'), /^system-/.test(msg.from) && (html.addClass('from-' + msg.from), html.find('.name').remove(), html.find('.avatar').remove()), html.find('.name').text(msg.name), html.attr('data-from', msg.from), html.find('.date').text(msg.date), html.attr('data-stamp', msg.timestamp), messages_dom.append(html), groupmsg_dom = html),
    msg.body = $('<pre></pre>').text(msg.body).html();
    var message = $('<div class="message"></div>').html(msg.body);
    return groupmsg_dom.append(message),
    groupmsg_dom == html ? messages_dom.scrollTop(messages_dom.scrollTop() + groupmsg_dom.outerHeight() + 10)  : messages_dom.scrollTop(messages_dom.scrollTop() + message.outerHeight()),
    message
  },
  ...
  pfc.showDonationPopup = function (next) {
    function buildAndShowDonationPopup() {
      var box = pfc.modalbox.open('<form class="popup-donate">  <div clalss="bt-validate">    <input type="submit" name="cancel-donate" value="Entrar" />  </div> </form>');
      ...
      box.find('input[type=submit]').click(function () {
        'ok-donate' == $(this).attr('name') && window.open('http://www.phpfreechat.net/donate', 'pfc-donate'),
        box.find('input[name=skip-donate]').attr('checked') && $.ajax({
          type: pfc.options.use_post_wrapper ? 'POST' : 'PUT',
        ...
        pfc.modalbox.close(!0),
        $(document).off('keyup', esc_key_action),
        next()
      }),
  ...
  pfc.displayError = function (err) {
    switch (err = err.responseText ? JSON.parse(err.responseText)  : {
      ...
      case 40305:
        err.baninfo.timestamp = new Date(1000 * err.baninfo.timestamp),
        pfc.appendMessage({
          type: 'error',
...
phpFreeChat = function (pfc, $, window) {
  return pfc.init = function (plugin) {
    ...
    pfc.uid = null,
    pfc.cid = null,
    (!pfc.options.check_backlink || pfc.hasBacklink()) && (pfc.loadHTML(), pfc.loadResponsiveBehavior(), pfc.checkServerConfig(pfc.startChatLogic))
  },
  ...
  pfc.checkServerConfigPHP = function (next) {
    $.ajax({
      ...
      pfc.showErrorsPopup(['Erro desconhecido: check.php não pode ser encontrado'])
    })
  },
  ...
  pfc.startChatLogic = function () {
    ...
    $(pfc.element).bind('pfc-login', function (evt, pfc, userdata) {
      ...
      pfc.cid = 'xxx',
      pfc.options.focus_on_connect && $('div.pfc-compose textarea').focus(),
      pfc.readPendingMessages(!0),
      pfc.join(pfc.cid)
    }),
  ...
  pfc.loadHTML = function () {
    $(pfc.element).html((pfc.options.loadTestData ? '      <div class="pfc-content">' : '      <div class="pfc-content pfc-notabs">') + '        <div class="pfc-tabs">' + '          <ul>' + (pfc.options.loadTestData ? '            <li class="channel active">              <div class="icon"></div>              <div class="name">Channel 1</div>              <div class="close"></div>            </li>            <li class="channel">              <div class="icon"></div>              <div class="name">Channel 2</div>              <div class="close"></div>            </li>            <li class="pm">              <div class="icon"></div>              <div class="name">admin</div>              <div class="close"></div>            </li>            <li class="new-tab">              <div class="icon"></div>            </li>' : '') + '          </ul>' + '        </div>' + '        <div class="pfc-topic">' + '          <a class="pfc-toggle-tabs"></a>' + '          <p><span class="pfc-topic-label">Topic:</span> <span class="pfc-topic-value">no topic for this channel</span></p>' + '          <a class="pfc-toggle-users"></a>' + '        </div>' + '        <div class="pfc-messages">' + '          <div class="pfc-message-mobile-padding"></div>' + (pfc.options.loadTestData ? '          <div class="messages-group" data-stamp="1336815502" data-from="kerphi">            <div class="avatar"><img src="http://www.gravatar.com/avatar/ae5979732c49cae7b741294a1d3a8682?d=wavatar&s=30" alt="" /></div>            <div class="date">11:38:21</div>            <div class="name">kerphi</div>            <div class="message">123 <a href="#">test de lien</a></div>            <div class="message">456</div>          </div>          <div class="messages-group" data-stamp="1336815503" data-from="admin">            <div class="avatar"><img src="http://www.gravatar.com/avatar/00000000000000000000000000000001?d=wavatar&s=30" alt="" /></div>            <div class="date">11:38:22</div>            <div class="name">admin</div>            <div class="message">Hello</div>            <div class="message">World</div>            <div class="message">!</div>            <div class="message">A very very very very very very very very very very very very very very very very very very very long text</div>          </div>' : '') + '        </div>' + '        <div class="pfc-users">' + '          <div class="pfc-role-admin">' + '            <p class="role-title">Usuário</p>' + '            <ul>' + (pfc.options.loadTestData ? '              <li class="first">                <div class="status st-active"></div>                <div class="name">admin</div>                <div class="avatar"><img src="http://www.gravatar.com/avatar/00000000000000000000000000000001?d=wavatar&s=20" alt="" /></div>              </li>' : '') + '            </ul>' + '          </div>' + '          <div class="pfc-role-user">' + '            <p class="role-title">Usuário</p>' + '            <ul>' + (pfc.options.loadTestData ? '              <li class="first">                <div class="status st-active"></div>                <div class="name myself">kerphi</div>                <div class="avatar"><img src="http://www.gravatar.com/avatar/ae5979732c49cae7b741294a1d3a8682?d=wavatar&s=20" alt="" /></div>              </li>              <li>                <div class="status st-inactive"></div>                <div class="name">Stéphane Gully</div>                <div class="avatar"><img src="http://www.gravatar.com/avatar/00000000000000000000000000000002?d=wavatar&s=20" alt="" /></div>              </li>' : '') + '            </ul>' + '          </div>' + '        </div>' + '        <div class="pfc-footer">' + (pfc.options.show_powered_by ? '          <p class="logo"><a href="http://www.phpfreechat.net" class="bt-powered" target="_blank">Powered by phpFreeChat</a><a href="http://www.phpfreechat.net/donate" class="bt-donate" title="Phpfreechat is for you and needs you" target="_blank">Donate</a></p>' : '') + (pfc.options.loadTestData ? '          <p class="ping">150ms</p>          <ul>            <li><div class="logout-btn" title="Logout"></div></li>            <li><div class="smiley-btn" title="Not implemented"></div></li>            <li><div class="sound-btn" title="Not implemented"></div></li>            <li><div class="online-btn"></div></li>          </ul>' : '') + '        </div>' + '        <div class="pfc-compose">' + '          <textarea placeholder="Digite aqui" data-to="channel|xxx"></textarea>' + '        </div>' + '        <div class="pfc-modal-overlay"></div>' + '        <div class="pfc-modal-box"></div>' + '        <div class="pfc-ad-desktop"></div>' + '        <div class="pfc-ad-tablet"></div>' + '        <div class="pfc-ad-mobile"></div>' + '      </div>'),
    pfc.options.show_powered_by && $.ajax({
      type: 'GET',
    ...
    pfc.modalbox.init(),
    pfc.options.loaded && pfc.options.loaded(pfc),
    setTimeout(function () {
      $(pfc.element).trigger('pfc-loaded', [
        pfc
      ])
    }, 0)
  },
  ...
  pfc.loadResponsiveBehavior = function () {
    function switchTabsToMobileLook() {
      ...
      elt_tabs.hide(),
      1 == tab_slide_status && (slideTabsUp(), tab_slide_status = 0)
    }
    ...
    function slideTabsDown(withtabs) {
      withtabs && elt_tabs.slideDown(500),
      elt_messages.animate({
        top: '+=' + height_slidetabs
      }, 500),
      elt_users.animate({
    ...
    $(window).resize(function () {
      elt_toggle_tabs_btn.is(':visible') ? (switchTabsToMobileLook(), scrollMessagesToBottom())  : switchTabsToDesktopLook(),
      elt_toggle_users_btn.is(':visible') ? (switchUsersToMobileLook(), scrollMessagesToBottom())  : switchUsersToDesktopLook()
    }),
    elt_toggle_tabs_btn.click(function () {
      elt_tabs.removeClass('pfc-tabs').addClass('pfc-mobile-tabs'),
      height_slidetabs = elt_tabs.height(),
      elt_tabs.is(':visible') ? (tab_slide_status = 0, slideTabsUp(!0))  : (tab_slide_status = 1, slideTabsDown(!0))
    }),
...
phpFreeChat = function (pfc, $, window) {
  ...
  pfc.md5 = function (s) {
    ...
    function K(G, k) {
      ...
      I = 1073741824 & G,
      d = 1073741824 & k,
      x = (1073741823 & G) + (1073741823 & k),
      I & d ? 2147483648 ^ x ^ F ^ H : I | d ? 1073741824 & x ? 3221225472 ^ x ^ F ^ H : 1073741824 ^ x ^ F ^ H : x ^ F ^ H
    }
    function r(d, F, k) {
      return d & F | ~d & k
    }
    ...
    function u(G, F, aa, Z, k, H, I) {
      return G = K(G, K(K(r(F, aa, Z), k), I)),
      K(L(G, H), F)
    }
    function f(G, F, aa, Z, k, H, I) {
      return G = K(G, K(K(q(F, aa, Z), k), I)),
      K(L(G, H), F)
    }
    function D(G, F, aa, Z, k, H, I) {
      return G = K(G, K(K(p(F, aa, Z), k), I)),
      K(L(G, H), F)
    }
    function t(G, F, aa, Z, k, H, I) {
      return G = K(G, K(K(n(F, aa, Z), k), I)),
      K(L(G, H), F)
    }
    function e(G) {
      for (var Z, F = G.length, x = F + 8, k = (x - x % 64) / 64, I = 16 * (k + 1), aa = Array(I - 1), d = 0, H = 0; F > H; ) Z = (H - H % 4) / 4,
      d = 8 * (H % 4),
    ...
    R = 15,
    O = 21;
    for (s = J(s), C = e(s), Y = 1732584193, X = 4023233417, W = 2562383102, V = 271733878, P = 0; C.length > P; P += 16) h = Y,
    E = X,
    v = W,
    g = V,
  ...
  var Base64 = {
    ...
    encode: function (e) {
      ...
      t = '',
      f = 0;
      for (e = Base64._utf8_encode(e); e.length > f; ) n = e.charCodeAt(f++),
      r = e.charCodeAt(f++),
      i = e.charCodeAt(f++),
      s = n >> 2,
      o = (3 & n) << 4 | r >> 4,
    ...
    decode: function (e) {
      ...
      t = '',
      f = 0;
      for (e = e.replace(/[^A-Za-z0-9\+\/\=]/g, ''); e.length > f; ) s = this._keyStr.indexOf(e.charAt(f++)),
    ...
    _utf8_decode: function (e) {
      for (var t = '', n = 0, r = c1 = c2 = 0; e.length > n; ) r = e.charCodeAt(n),
  ...
  return pfc.base64 = Base64,
  pfc.modalbox = {
    open: function (html) {
      return html = $(html),
      $('div.pfc-modal-box *').remove(),
      $('div.pfc-modal-box').append(html).fadeIn(),
      $.browser.msie ? $('div.pfc-modal-overlay').show()  : $('div.pfc-modal-overlay').fadeIn('fast'),
